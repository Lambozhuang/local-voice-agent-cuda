...
from tts_kokoro_cuda import KokoroCUDATTSService
...
async def run_bot(webrtc_connection):
    ...
    # Whisper on the NVIDIA GPU via faster-whisper / CTranslate2.
    # compute_type "int8_float16" mirrors the quantized MLX LARGE_V3_TURBO_Q4 path;
    # use "float16" for full-precision GPU inference.
    ...
    tts = KokoroCUDATTSService(voice="af_heart", device="cuda", sample_rate=24000)

    llm = OpenAILLMService(
        api_key="dummyKey",
        model="gemma-3n-e4b-it-text",  # Small model. Uses ~4GB of VRAM.
        # model="google/gemma-3-12b",  # Medium-sized model. Uses ~8.5GB of VRAM.
        # Use whatever model name your local LLM server (vLLM/Ollama/llama.cpp/LM Studio) exposes.
